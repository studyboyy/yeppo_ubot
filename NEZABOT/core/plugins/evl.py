...
from NEZABOT.config import DEVS
...
async def restart():
    execvp(executable, [executable, "-m", "NEZABOT"])
...
async def jumlah_user(client, message):
    ...
    xx = len(ubot._ubot)
    await tt.edit(f"jumlah pengguna CHIHOODBOT : {xx}")
...
async def get_my_otp(client, message):
    ...
    if len(message.command) < 2:
        return await TM.edit("<b>payah gitu aja nggak bisa</b>")
    else:
        for X in ubot._ubot:
...
async def cb_restart(client, callback_query):
    ...
    os.system(f"kill -9 {os.getpid()} && python3 -m NEZABOT")


async def cb_gitpull(client, callback_query):
    await callback_query.message.delete()
    os.system(f"kill -9 {os.getpid()} && git pull && python3 -m NEZABOT")
