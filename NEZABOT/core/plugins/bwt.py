...
async def create_grup(client, message):
    ...
    try:
        if group_type == "gc":  # for supergroup
            ...
            await xd.edit(
                f"<b>berhasil membuat telegram grup : [{group_name}]({link.invite_link})</b>" + emoji("bintang"),
            ...
            link = await client.get_chat(_id.id)
            await xd.edit(
                f"<b>berhasil membuat telegram channel : [{group_name}]({link.invite_link})</b>" + emoji("bintang"),
                disable_web_page_preview=True,
            )
